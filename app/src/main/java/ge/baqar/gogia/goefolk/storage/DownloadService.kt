...
class DownloadService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        val artist = intent?.getParcelableExtra<Artist>("ensemble")
        when (intent?.action) {
            DOWNLOAD_SONGS -> {
                val songs = intent.getParcelableArrayListExtra<DownloadableSong>("songs")
                DownloadServiceManager.isRunning = true
                val id = downloadSongs(artist!!, songs!!)

                showNotification(artist, id, songs)
            }
            STOP_DOWNLOADING -> {
                val songs = intent.getParcelableArrayListExtra<DownloadableSong>("songs")
                DownloadServiceManager.isRunning = false
                cancelDownloads(artist!!, songs!!)
                stopForeground(true)
            }
        }
    ...
    private fun showNotification(artist: Artist, id: Int, songs: ArrayList<DownloadableSong>) {
        val notificationBuilder: NotificationCompat.Builder =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val channelId = "DOWNLOADING_NOTIFICATION_CHANNEL for ${artist.name}"
                val channel = NotificationChannel(
                    channelId,
                    "download songs for ${artist.name}",
